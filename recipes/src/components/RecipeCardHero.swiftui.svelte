...
<navigationlink>
	...
	<vstack template="label" spacing="0" modifiers="marginBottom(24);">
		...
		<vstack alignment="leading" spacing="12" modifiers="padding(20);marginTop(-40);background(.background);cornerRadius(16);shadow(.drop(radius: 8, x: 0, y: 4));">
			...
			<!-- CTA hint -->
			<hstack modifiers="frame(maxWidth: .infinity);padding(.vertical, 14);background(.orange.opacity(0.1), in: .rect(cornerRadius: 12));padding(.top, 4);">
				<spacer />
				<hstack spacing="8">
					<text modifiers="font(.headline);fontWeight(.bold);foregroundColor(.orange);">View Recipe</text>
					<image systemname="arrow.right" modifiers="font(.subheadline);fontWeight(.bold);foregroundColor(.orange);" />
				</hstack>
				<spacer />
			</hstack>
		</vstack>
